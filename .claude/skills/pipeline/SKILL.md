---
name: pipeline
description: End-to-end source processing -- seed, distill, process all claims through connect/revisit/verify, archive. The full pipeline in one command. Triggers on "/pipeline", "/pipeline [file]", "process this end to end", "full pipeline".
version: "1.0"
...
Phase 1: /seed — create extract task, move source to archive
    |
    v
Phase 2: /distill (via /ralph) — extract claims from source
    |
    v
...
Process the extract task via /ralph. This spawns a subagent that runs /distill, extracting claims from the source and creating task entries in the queue.
...
Check how many pending tasks exist for this batch. The distill phase creates 1 queue entry per claim and 1 per enrichment.
...
Processing:
  ...
  Connections added: {C}
  Maps updated: {T}
  Older notes updated via revisit: {R}
...
**Extract failure:** If /distill extracts zero claims, report and stop. Do not proceed to an empty processing phase.
...
| After distill, during claims | /ralph --batch {id} (picks up from failed phase) |
