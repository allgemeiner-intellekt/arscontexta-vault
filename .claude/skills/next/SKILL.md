...
1. **`ops/derivation-manifest.md`** — vocabulary mapping, domain context
   ...
   - Use `vocabulary.topic_map` for map references
   - Use `vocabulary.cmd_reduce` for distill command
...
Read `ops/derivation-manifest.md` (or fall back to `ops/derivation.md`) for domain vocabulary mapping. All output must use domain-native terms. If neither file exists, use universal terms (notes, inbox, map, etc).
...
| moc_oversize | For each map, count linked notes. |
...
| **Slow** | Health check not run in 14+ days, map oversized (>40 notes), link density below 2.0 average, low note count relative to time | Background — annoying but not blocking | These are maintenance tasks. Important for long-term health but not urgent. |
...
- If inbox pressure AND queue backlog: recommend distilling inbox first (pipeline needs input before it can process)
...
| Inbox > 5 items | /distill [specific file] | "Your inbox has [N] items (oldest: [age]). [File X] has the highest connection potential based on [reason]." |
...
| Signal | Recommendation | Rationale Template |
|--------|---------------|-------------------|
| Queue backlog > 10 | /ralph [N] | "[N] pipeline tasks are pending. Your newest notes lack connections, which means they can't participate in synthesis." |
...
| Research gaps | /distill [file aligned with goals] | "Your goals mention [topic] but your graph has few notes there. [Inbox item] addresses this gap." |
...
| Map oversized | Restructuring suggestion | "[Map X] has [N] notes. Splitting into sub-maps improves navigation and reduces cognitive load." |
...
| Good | Bad |
|------|-----|
| `/distill inbox/article-on-spaced-repetition.md` | "process some inbox items" |
...
Recommend capturing or distilling content. Maintenance is premature with < 5 notes — the graph does not have enough nodes for meaningful analysis.
