...
- Domain name and index map name
...
Research complete
  ...
  Next: /distill inbox/[filename]
```

Append based on mode:
- **manual:** (nothing extra)
- **suggested:** `Ready for processing when you are.`
- **automatic:** Replace "Next" line with `Queued for /distill -- processing will begin automatically.`
...
| Phase | Skill | Purpose |
|-------|-------|---------|
| Extract insights | /distill | Mine research for atomic propositions |
| Find connections | /connect | Link new insights to existing graph |
| Update old notes | /revisit | Backward pass on touched notes |
