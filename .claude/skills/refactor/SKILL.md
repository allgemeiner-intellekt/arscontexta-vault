...
|--------|-------------------|-------------|
| **Granularity shift** | Note templates, extraction depth in /distill, processing skills, context file "Note Design" section | Template body length guidance, extraction granularity settings, composability test thresholds |
| **Organization shift** | Folder structure, map hierarchy, context file "Folder Architecture" section, index map | Directory layout, map tier count, navigation references |
...
| **Navigation shift** | Map tier structure, index map, context file "Map" section, note Topics footers | Number of map tiers, index content, navigation instructions |
| **Maintenance shift** | /health threshold values, condition-based trigger settings, context file maintenance instructions | Check frequency conditions, stale note thresholds, revisit trigger conditions |
...
Some changes affect only infrastructure (skills, templates, context file). Others affect existing content (notes, maps):
...
| Map tier change | Map hierarchy restructured | Merge/split maps, update Topics footers |
...
|-----------------|---------------------|
| Granularity | /distill, /verify, /validate |
| Processing | /distill, /connect, /revisit, /verify, /ralph, /pipeline |
| Linking | /connect, /revisit |
| Navigation | /connect (map update logic) |
...
4. **Map restructuring:** Merge/split maps as needed
   - For splits: create sub-maps, move Core Ideas, update parent
   - For merges: combine Core Ideas, remove redundant map, update notes' Topics footers
...
| Map hierarchy | Verify index links to domain maps, domains link to topics | All tiers connected |
