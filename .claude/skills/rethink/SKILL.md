...
|-------------|---------|---------------|--------|
| PROMOTE | Reusable insight worth keeping as a permanent note | General principle across sessions. Would work as a note. Crystallized insight, not operational guidance. | Create note in notes/, set observation `status: promoted`, add `promoted_to: [[title]]` |
...
- Tension reveals a genuine conflict between two notes → PROMOTE (create a tension note or resolution note)
...
2. Follow standard note schema: YAML frontmatter (description, type, created), body developing the insight, Topics footer linking to relevant map(s)
...
2. **Group by referenced maps or system areas:** Extract wiki links and file references from observation bodies. 3+ observations referencing the same area = recurring theme.
...
- Add each promoted note to the queue with `current_phase: "connect"` (the note already exists, so skip create)
