---
name: connect
description: Find connections between notes and update maps. Requires semantic judgment to identify genuine relationships. Use after /distill creates notes, when exploring connections, or when a topic needs synthesis. Triggers on "/connect", "/connect [note]", "find connections", "update maps", "connect these notes".
user-invocable: true
...
1. **`ops/derivation-manifest.md`** — vocabulary mapping, platform hints
   ...
   - Use `vocabulary.topic_map` / `vocabulary.topic_map_plural` for map references
...
|-------|-------------------|
| deep | Full dual discovery (map + semantic search). Evaluate every candidate. Multiple passes. Synthesis opportunity detection. Bidirectional link evaluation for all connections. |
| standard | Dual discovery with top 5-10 candidates. Standard evaluation. Bidirectional check for strong connections only. |
| quick | Single pass — either map or semantic search. Accept obvious connections only. Skip synthesis detection. |
...
2. **Throughout discovery:** Capture which maps you read, which queries you ran (with scores), which candidates you evaluated. This becomes the Discovery Trace — proving methodology was followed, not reconstructed.
3. Run Phase 0 (index freshness check)
4. Use dual discovery in parallel:
   - Browse relevant map(s) for related notes
...
7. Update relevant map(s) with this note
...
Find connections, weave the knowledge graph, update maps. This is the forward-connection phase of the processing pipeline.
...
3. Add connections and update maps

### /connect [topic area]

Synthesize an area:
1. Read the relevant map
...
Before using semantic search, verify the index is current. This is self-healing: if notes were created outside the pipeline (manual edits, other skills), connect catches the drift before searching.
...
**If a task file exists** (pipeline execution): read the task file to see what the extraction phase discovered. The distill notes, semantic neighbor field, and classification provide critical context about why this note was extracted and what it relates to.

### Phase 2: Discovery (Find Candidates)

Use dual discovery: map exploration AND semantic search in parallel. These are complementary, not sequential.

**Capture discovery trace as you go.** Note which maps you read, which queries you ran (with scores), which searches you tried. This becomes the Discovery Trace section in output — proving methodology was followed, not reconstructed after the fact.

**Primary discovery (run in parallel):**

**Path 1: Map Exploration** — curated navigation

If you know the topic (check the note's Topics footer), start with the map:

- Read the relevant map(s)
...
Maps tell you what thinking exists and how it is organized. Someone already decided what matters for this topic.

**Path 2: Semantic Search** — find what maps might miss
...
**Tier 3 — grep only:** If both MCP and bash fail, log "qmd unavailable, grep-only discovery" and rely on map + keyword search only. This degrades quality but does not block work.
...
map = what is already curated as relevant
semantic search = neighbors that have not been curated yet

Using only search misses curated structure. Using only map misses semantic neighbors outside the topic. Both together catch what either alone would miss.
...
When you edit an older note to add a reverse link, you MAY flag it for full reconsideration via revisit. But SKIP revisit flagging if ANY of these apply:
...
| Note was revisited in current batch | Do not re-revisit what was just revisited |
| Note is a map | Maps are navigation, not claims to reconsider |
...
Maps are synthesis hubs, not just indexes.
...
If approaching the split threshold (configurable, default ~40): note in output "map approaching split threshold (N links)"
If exceeding: warn "map exceeds recommended size — consider splitting"
...
[Opening synthesis: Claims about the topic. Not "this map collects notes" but "the core insight is Y because Z." This IS thinking, not meta-description.]
...
**Bad agent notes:**
```markdown
- 2026-02-15: read the map and added some links.
...
### Gate 4: Map Coherence

After updating a map, read the opening synthesis. Does it still hold? Do new notes extend or challenge it?
...
1. Ensure it is linked to at least one map via Topics footer
2. Note in map Gaps that this area needs development
...
1. **Domain spread:** Connections span 3+ distinct maps/topic areas
...
3. If genuinely orphaned, note in relevant map Gaps
...
**Keyword search:**
- grep "specific term" — found [[note H]] (already in map candidates)
...
- Maps gain synthesis, not just entries
...
Work Done:
- Discovery: map [[moc-name]], query "[query]" (MCP|bash|grep-only), grep "[term]"
- Connections added: N (articulation test: PASS)
- Map updates: [[moc-name]] Core Ideas section
...
- Revisit candidates (if any pass filter): [[note]] | NONE (filtered: hub/tension/recent)
=== END HANDOFF ===
```
