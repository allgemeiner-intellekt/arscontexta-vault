...
|-------|-----------------------|
| deep | Full verification: cold-read prediction, complete schema check, exhaustive link verification, map coverage, orphan risk analysis, content staleness detection, bundling analysis |
| standard | Balanced: cold-read prediction, schema check, link verification, map coverage |
...
4. COMPLETE map integration — verify the note appears in at least
   one map's Core Ideas section with a context phrase.
   A note with no map mention FAILS.
...
**Why vector_search specifically:** Agents find notes via semantic search during connect and revisit. Testing with keyword search tests the wrong retrieval method. Full hybrid search with LLM reranking compensates for weak descriptions — too lenient. vector_search tests real semantic findability without hiding bad descriptions behind reranking.
...
| Topics footer or `topics` field | Must reference at least one map | FAIL |
...
| Links exist | Each map must exist as a file | WARN |
...
**3. Map connection**
- Note appears in at least one map's Core Ideas section
- How to check: grep for `[[note title]]` in files that serve as maps
- The note's Topics footer references a valid map
- A note with no map mention is orphaned — FAIL
...
- Missing Topics footer (if obvious which map applies)
...
- Ambiguous map assignment (when note could fit multiple)
...
REVIEW:
  ...
  Map connection: [PASS/FAIL — which map]
...
3. **Map connection** — appears in at least one map
...
| Missing map | Review fails map check | Add to appropriate map or create Topics footer |
