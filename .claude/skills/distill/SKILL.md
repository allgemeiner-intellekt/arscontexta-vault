---
name: distill
description: Extract structured knowledge from source material. Comprehensive extraction is the default — every insight that serves the domain gets extracted. For domain-relevant sources, skip rate must be below 10%. Zero extraction from a domain-relevant source is a BUG. Triggers on "/distill", "/distill [file]", "extract insights", "mine this", "process this".
version: "1.0"
...
Extract composable notes from source material into notes/.
...
|----------|--------------|-------------|----------------|
| Core domain notes | Direct assertions about the domain | note | NO |
...
|----------|-----------------|----------|
| Core domain note | Direct assertion about the domain | -> note (SKIP selectivity gate) |
...
**The enrichment default:** When in doubt between "new note" and "enrichment to existing note", lean toward enrichment. The existing note already has connections, map placement, and integration. Adding to it compounds existing value.
...
- Map notes (sections: Synthesis, Core Ideas, Tensions, Explorations Needed, Agent Notes)
...
- The chain: research query -> inbox file -> /distill -> notes
