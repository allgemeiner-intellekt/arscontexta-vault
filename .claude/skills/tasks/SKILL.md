...
Discoveries are captured during pipeline work (e.g., /distill notes a connection opportunity, /connect notices a split candidate). They accumulate here until the user decides to convert them to tasks or discard them.
...
- `/distill` adds tasks to the queue AND notes discoveries in tasks.md
...
## Discoveries
- Interesting connection between [[note A]] and [[note B]] found during /distill
- Map [[topic]] might need splitting (40+ notes observed during /connect)
```
