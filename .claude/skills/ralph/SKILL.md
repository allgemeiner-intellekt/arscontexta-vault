...
|-------|---------------|---------|
| extract | /distill | Extract claims from source material |
| create | (inline note creation) | Write the note file |
| enrich | /enrich | Add content to existing note |
| connect | /connect | Find connections, update maps |
| revisit | /revisit | Update older notes with new connections |
...
Create a note for this claim in notes/[claim as sentence].md
...
Use dual discovery: map exploration AND semantic search.
Add inline links where genuine connections exist — including sibling claims listed above.
Update relevant map with this note.
...
**For extract tasks ONLY:** Re-read the queue after marking done. The distill skill writes new task entries (1 entry per claim/enrichment with `current_phase`/`completed_phases`) to the queue during execution. The lead must pick these up for subsequent iterations.
...
because sibling notes did not exist yet when the earlier claim's connect ran.
...
**Why two phases?** Workers have sibling awareness (claim titles in spawn prompt) and link proactively during connect/revisit. But timing means some sibling notes may not exist yet during a worker's connect phase. Phase B runs a single cross-connect pass after all notes exist.
...
If a sibling note exists in notes/, link to it inline where the
...
workers missed because sibling notes did not exist yet when a worker's connect ran.
Check backward link gaps. Output RALPH HANDOFF block when done.",
  description = "cross-connect: batch {BATCH}"
)
```
