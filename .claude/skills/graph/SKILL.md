...
2. **`ops/config.yaml`** — for graph thresholds (map size limits, orphan thresholds)

If no derivation file exists, use universal terms (notes, maps, etc.).
...
```bash
# Count total notes (excluding maps)
NOTES_DIR="notes"
...
# Map coverage: % of notes appearing in at least one map's Core Ideas
COVERED=0
for f in "$NOTES_DIR"/*.md; do
  NAME=$(basename "$f" .md)
  # Skip maps themselves
  grep -q '^type: moc' "$f" 2>/dev/null && continue
  # Check if any map links to this note
...
--=={ graph health }==--
  ...
  Graph density: [0.XX]
  map coverage: [N]% of notes appear in at least one map
...
- Both are in the same map (they may already be related through the map without direct links)
- One is a map itself (maps link to everything, triangles with maps are noise)
...
- Topic coverage (which maps are represented)
...
Find unconnected notes within a topic — notes sharing the same map but not linking to each other.

**Step 1: Read the specified map**

Find and read the map matching the argument. Extract all notes linked in Core Ideas.

**Step 2: Check pairwise connections**

For each pair of notes in the map:
...
- **Use domain vocabulary** for all labels and descriptions — note, map, etc.
...
Use universal vocabulary (notes, maps, etc.). All operations work identically.

### Empty Notes Directory

Report: "No notes found in notes/. Start by capturing content to build your knowledge graph."

### Note Not Found (for forward/backward/siblings)

If the specified note or map does not exist:
