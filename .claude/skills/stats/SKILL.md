...
--=={ stats }==--
  ...
  maps:   [MOC_COUNT] (covering [COVERAGE]% of notes)
  Topics:                    [TOPIC_COUNT]
...
- **[MOC_COUNT]** maps covering [COVERAGE]% of notes
...
Use universal vocabulary (notes, maps, etc.). All metrics work identically.

### Very Large Vault (500+ notes)

The orphan and map coverage checks may be slow for large vaults. If notes/ has >200 files:
1. Run orphan detection with a simpler heuristic (check only for presence in any map, not full backlink scan)
